...
export default async function ProfilePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-7 bg-gray-900 text-white shadow-md rounded-lg mt-6 transition-all duration-300 font-inter">
      ...
      <p className="text-xs text-gray-600 dark:text-gray-400 mt-2 font-inter">
        VISUALIZAR INFORMACION DE USUARIO
      </p>
      ...
      <div className="flex flex-col items-center text-center mb-6 mt-6 font-inter">
        {user.image ? (
          ...
          <div className="w-28 h-28 flex items-center justify-center rounded-full shadow-lg mb-2 border-4 border-blue-500 bg-gray-700 text-white text-3xl font-bold transform transition duration-300 hover:scale-110">
            {getInitials(user.name)}
          </div>
        )}
        <div className="text-blue-400 flex items-center cursor-pointer mt-0 font-inter">
          <FaEdit className="mr-2" />
          <span>Editar foto de perfil</span>
        </div>
      </div>

      <div className="mt-4 p-5 bg-gray-800 rounded-lg space-y-4 font-inter">
        ...
        {user.status !== undefined && (
          <div className="flex items-center transform transition duration-300 hover:scale-105">
            {user.status ? (
              ...
              <FaRegTimesCircle className="mr-2 text-red-400" />
            )}
